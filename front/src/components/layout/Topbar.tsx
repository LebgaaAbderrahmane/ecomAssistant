...
import { Store, ChevronDown, Settings as SettingsIcon, CreditCard, LogOut } from 'lucide-react'
...
export function Topbar() {
  ...
  return (
    <header className="sticky top-0 z-50 flex h-14 items-center justify-between border-b border-gray-200 bg-white px-8">
      ...
      <div ref={ref} className="relative">
        ...
        >
          ...
          <span className="text-sm text-gray-700">{user?.email || 'marchand@email.com'}</span>
        ...
        {dropdownOpen && (
          <div className="absolute right-0 top-full mt-1 min-w-[180px] rounded-lg border border-gray-200 bg-white shadow-md">
            ...
            <button
              onClick={() => { navigate('/dashboard/billing'); setDropdownOpen(false) }}
              className="flex h-10 w-full items-center gap-3 px-4 text-sm text-gray-700 hover:bg-gray-50"
            >
