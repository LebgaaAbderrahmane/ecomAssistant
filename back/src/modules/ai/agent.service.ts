...
import { IntentSchema, ReadToolNameSchema, WriteToolNameSchema, resolveTool, isSuggestedIntent, isReadTool, isWriteTool, intentToString, type IntentField, type ToolName, type ReadToolName, type SuggestedIntentField } from './schemas/intents.schemas';
...
//   + bare product references (name → id) resolved from the customer's own
//     message before createOrder/selectProduct, so a tool never reads
...
async function executeTools(
  ...
  // Whether an exact product request (search) succeeded this message.
  ...
  for (const item of intents) {
    ...
    // resolve the reference against the catalog and only report NOT_FOUND/
    // AMBIGUOUS when the request cannot identify a product. getProductDetails /
    // selectProduct can also fall back to the injected currentProductId.
    ...
    // Enrich productId from product name for product intents that consume an
    // explicit product reference. searchProducts is excluded: its `product`
    // argument is a free-text catalog query, not a reference to resolve.
    const entities = { ...item.entities };

    if (toolName !== 'searchProducts' && entities.product && !entities.productId) {
    ...
    const toolEntities: InjectedToolEntities = buildToolEntities(toolName, entities, source, {
      lastProductResults,
    });
    ...
    // Post-processing: store search results in memory for follow-up selection
    // (PRODUCT_SELECT) and suggestions exclusions.
    if (
      toolName === 'searchProducts' &&
      result.success &&
  ...
  for (const { item, entities, toolKey } of deferredSuggestions) {
    ...
    if (toolResultCache.has(toolKey)) {
      ...
      // suggestProducts consumes only explicit criteria + the transport-computed
      // exclusions (abandoned/rejected/current products). Pass the in-message
      // lastProductResults so products offered earlier THIS message are never
      // re-suggested — the tool itself never reads memory.
      const suggestionEntities = buildToolEntities('suggestProducts', entities, source, {
        lastProductResults,
      });
      try {
...
export const processMessage = async (messageId: string) => {
  ...
  if (suggestedItems.length > 0) {
    ...
    const uniqueSuggested: SuggestedIntentField[] = [];
    for (const item of suggestedItems) {
      ...
      uniqueSuggested.push(suggested);
      try {
    ...
    if (!takenOver) {
      const firstSuggested = uniqueSuggested[0];
      const result = await executeTool('escalateConversation', {
        merchantId: conversation.merchantId,
        customerId: conversation.customerId,
        conversationId: conversation.id,
        reason: firstSuggested
          ? `Suggested intent: ${firstSuggested.name} — ${firstSuggested.description}`
          : 'Unhandled request requires a human agent',
      });
